...
class ChromebookScheduler:
    ...
    def setup_ui(self):
        ...
        hud_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        ...
        # Data de retirada
        ttk.Label(form_frame, text="Data de Retirada (DD/MM/AAAA):").grid(row=1, column=2, sticky=tk.W, pady=5)
        self.data_entry = ttk.Entry(form_frame, width=30)
        self.data_entry.grid(row=1, column=3, sticky=(tk.W, tk.E), pady=5)
    ...
    def atualizar_hud_disponiveis(self):
        """Atualiza o HUD com a quantidade disponível"""
        try:
            df = pd.read_excel(self.filename)
            total_agendado = df['Quantidade de Chromebooks'].sum() if not df.empty else 0
            disponivel = self.total_disponivel - total_agendado
            self.disponivel_label.config(text=f"Chromebooks Disponíveis: {disponivel}")
            
            # Atualizar o limite máximo do spinbox
            self.quantidade_spinbox.config(to=disponivel)
    ...
    def add_schedule(self):
        ...
        disponivel = self.atualizar_hud_disponiveis()
        
        if quantidade_int > disponivel:
            messagebox.showerror("Erro", f"Quantidade indisponível! Apenas {disponivel} chromebooks disponíveis.")
            return
